...
RSpec.describe 'Most revenue of a variable number of merchants' do
  before :each do
  ...
  it "can return the top three merchants ranked by total revenue" do
    get "/api/v1/merchants/most_revenue", params: { :quantity => 3 }

    most_rev_json = JSON.parse(response.body, symbolize_names: true)

    expect(response).to be_successful
    expect(response.content_type).to eq('application/json')
    expect(most_rev_json[:data][0]).to have_key(:id)
    expect(most_rev_json[:data][0]).to have_key(:type)
    expect(most_rev_json[:data][0]).to have_key(:attributes)
    expect(most_rev_json[:data][0][:attributes]).to have_key(:name)
    expect(most_rev_json[:data][0][:attributes][:name]).to eq(@m5.name)
    expect(most_rev_json[:data][1][:attributes][:name]).to eq(@m4.name)
    expect(most_rev_json[:data][2][:attributes][:name]).to eq(@m3.name)
  ...
  it "will just return all merchants if quantity is greated than amount in db" do
    get "/api/v1/merchants/most_revenue", params: { :quantity => 10 }
    ...
    expect(most_rev_json[:data][1][:attributes][:name]).to eq(@m4.name)
    expect(most_rev_json[:data][2][:attributes][:name]).to eq(@m3.name)
    expect(most_rev_json[:data][3][:attributes][:name]).to eq(@m2.name)
    expect(most_rev_json[:data][4][:attributes][:name]).to eq(@m1.name)
    expect(most_rev_json[:data][0][:attributes][:name]).to_not eq(@m6.name)
    expect(most_rev_json[:data][0][:attributes][:name]).to_not eq(@m7.name)
  end
